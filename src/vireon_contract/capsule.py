...
def write_capsule(out_dir: Path, capsule: Capsule, claim_file: Path | None = None) -> None:
    ...
    # Write capsule.json (stable)
    capsule_json = capsule.to_json().encode("utf-8")
    (out_dir / "capsule.json").write_bytes(capsule_json)

    # Copy claim.json into capsule (LAW §5 self-containment)
    if claim_file is not None:
        target = out_dir / "claim.json"
        target.write_bytes(Path(claim_file).read_bytes())

    # Hash everything currently present in the capsule directory
